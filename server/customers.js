...
router.get("/", async (req, res) => {
  try {
    ...
    return res.status(500).json({ ok: false, error: "read_failed", details: err.message || String(err) });
  }
});
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ ok: false, error: "append_failed", details: err.message || String(err) });
  }
});
